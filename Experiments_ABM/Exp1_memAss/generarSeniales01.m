function [membership,W,senial]=generarSeniales01(fil,col,M)
%initializar vars
nSegundos =20;
frecMuestreo = 128; %Hz
...
[SynthSignal,P,orSignals,SSignal]=SyntheticSignalGeneration(fil,col,M);
       ...
       P3{cc,2}=P1;
        ...
        for j=1:nSeniales
           ...
           mTemp(1,j)=sum(abs(temp))/length(temp);
            ...
            logmem=log(HRmem);
            for cc=1:nSeniales
                membership(6,cc)=1-abs(logmem(cc))/max(abs(logmem));
            end
